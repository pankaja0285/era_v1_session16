...
class OpusDataModule(LightningDataModule):
    ...
    def prune_dataset(self, train_ds_raw):
        """
        Trim the dataset so that the longest sentence is more than 150
        """
        ...
        # instead do this
        src_threshold_chars = self.config['src_threshold_chars']
        sorted_train_ds = sorted(train_ds_raw, 
        key=lambda x: (len(x["translation"][lang_src]) <= src_threshold_chars))   # 150
